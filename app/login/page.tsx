"use client";

import { Suspense, useState } from "react";
import { useSearchParams } from "next/navigation";
import { createClient } from "@/lib/supabase/client";
import {
  GOOGLE_OAUTH_QUERY_PARAMS,
  GOOGLE_OAUTH_SCOPES,
} from "@/lib/google-oauth";
import { Logo } from "@/components/logo";

function LoginContent() {
  const [loading, setLoading] = useState(false);
  const searchParams = useSearchParams();
  const hadError = searchParams.get("error") === "auth";

  async function signInWithGoogle() {
    setLoading(true);
    const supabase = createClient();
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback`,
        scopes: GOOGLE_OAUTH_SCOPES,
        queryParams: GOOGLE_OAUTH_QUERY_PARAMS,
      },
    });
    if (error) setLoading(false);
  }

  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-8 p-6">
      <div className="flex flex-col items-center text-center">
        <Logo size={76} />
        <h1 className="mt-4 text-3xl font-bold tracking-tight">
          Gift Card Snapper
        </h1>
        <p className="mt-2 text-sm opacity-70">
          Snap your gift cards. Get rows in Google Sheets.
        </p>
      </div>

      {hadError && (
        <p className="rounded-lg bg-red-100 px-4 py-2 text-sm text-red-800 dark:bg-red-950 dark:text-red-200">
          Sign-in didn&apos;t complete. Please try again.
        </p>
      )}

      <button
        onClick={signInWithGoogle}
        disabled={loading}
        className="flex w-full max-w-xs items-center justify-center gap-3 rounded-xl border border-black/15 bg-white px-6 py-4 text-base font-medium text-gray-900 shadow-sm transition active:scale-[0.98] disabled:opacity-60 dark:border-white/20"
      >
        <svg width="20" height="20" viewBox="0 0 48 48" aria-hidden="true">
          <path
            fill="#EA4335"
            d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"
          />
          <path
            fill="#4285F4"
            d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"
          />
          <path
            fill="#FBBC05"
            d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"
          />
          <path
            fill="#34A853"
            d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"
          />
        </svg>
        {loading ? "Redirecting…" : "Sign in with Google"}
      </button>

      <p className="max-w-xs text-center text-xs opacity-50">
        Your card details are never stored by this app — they go straight to
        a Google Sheet in your own Drive.
      </p>

      <p className="text-xs opacity-40">
        <a href="/privacy" className="underline">
          Privacy
        </a>
        {" · "}
        <a href="/terms" className="underline">
          Terms
        </a>
      </p>
    </main>
  );
}

export default function LoginPage() {
  return (
    <Suspense>
      <LoginContent />
    </Suspense>
  );
}
